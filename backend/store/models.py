...
class Product(models.Model):
    ...
    # ID of the source record (e.g. DummyJSON product id); NULL for manual products
    external_id         = models.PositiveBigIntegerField(null=True, blank=True, unique=True)
...
class ProductVariant(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Use product.slug (already unique + disambiguated with -<id> suffix on collision)
        # instead of product_name, so two products whose names differ only by "&" vs "and"
        # don't generate identical SKUs.
        product_slug = self.product_color.product.slug
        color_name = self.product_color.color.name

        self.sku = slugify(
            f"{product_slug}-{color_name}-{self.size.name}"
        ).upper()
